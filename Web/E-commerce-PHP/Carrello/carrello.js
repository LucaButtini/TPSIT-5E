...
function populateCartPage(cart) {
    ...
    cart.forEach((item, index) => {
        const itemTotal = item.price * item.quantity; //  il totale per il prodotto
...
function updateQuantity(index, change) {
    ...
    if (cart[index]) {
        ...
        // se quantità è 0 tolgo il prodotto dal carrello
...
function applyDiscount() {
    const discountCode = document.getElementById("discount-code").value.trim().toUpperCase(); //recupero il codice di sconto
    const discountCodes = {
